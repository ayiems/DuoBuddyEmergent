...
mount_api server, '/api/auth/signup' do |req, res|
  ...
  users = read_users
  email_down = email.to_s.downcase
  username_down = username.to_s.downcase
  if users.any? { |u| u['email'].to_s.downcase == email_down }
    res.status = 409; res.body = { error: 'email_exists' }.to_json; next
  end
  if users.any? { |u| u['username'].to_s.downcase == username_down }
    res.status = 409; res.body = { error: 'username_exists' }.to_json; next
  end
  now = Time.now
...
mount_api server, '/api/company/update' do |req, res|
  set_cors(req, res)
  if req.request_method == 'OPTIONS'
    res.status = 204; next
  end
  ...
  if req.request_method == 'POST'
    ...
    company['name'] = body['name'] if body['name']
    company['registrationNumber'] = body['registrationNumber'] if body['registrationNumber']
    company['industry'] = body['industry'] if body['industry']
    company['address'] = body['address'] if body['address']
    company['email'] = body['email'] if body['email']
    company['phone'] = body['phone'] if body['phone']
    company['contactPerson'] = body['contactPerson'] if body['contactPerson']
...
mount_api server, '/api/public/profile' do |req, res|
  set_cors(req, res)
  if req.request_method == 'OPTIONS'
    res.status = 204; next
  end
  set_json(res)
  username = req.path.sub('/api/public/profile/', '').strip
  username = username.downcase
  users = read_users
  user = users.find { |u| u['username'] && u['username'].downcase == username }
  if !user || user['deleted']
    res.status = 404; res.body = { error: 'not_found' }.to_json; next
  end
  profile = user['data'] ? JSON.parse(user['data']) : {}
  trial_end_raw = profile['trialEnd']
  trial_ok = false
  if trial_end_raw
    begin
      trial_time = Time.parse(trial_end_raw) rescue nil
      trial_ok = trial_time && Time.now < trial_time
    rescue
      trial_ok = false
    end
  end
  has_active_card = (profile['cards'] || []).any? { |c| c['status'] == 'active' || c['ordered'] }
  payment_approved = !!profile['paymentApproved']
  unless payment_approved || trial_ok
    res.status = 403
    res.body = { error: 'unavailable' }.to_json
    next
  end
  preferred_email = if profile['useLoginForPreview']
    user['email']
  else
    profile['preferredEmail'].to_s.strip.empty? ? user['email'] : profile['preferredEmail']
  end
  phone = user['phone']
  ...
  if host.downcase.include?('localhost') || host.include?('127.0.0.1')
    scheme = 'http'
  else
    scheme = 'https'
  end
  
  profile_url = "#{scheme}://#{host}/card/#{user['username']}"
  whatsapp_link = nil
  if whatsapp_number && !whatsapp_number.to_s.strip.empty?
    digits = whatsapp_number.gsub(/\D/, '')
    whatsapp_link = digits.empty? ? nil : "https://wa.me/+6#{digits}"
  end
  qr_profile = profile_url
  qr_whatsapp = whatsapp_link
  public_data = {
    name: user['name'],
    ...
    qr: {
      profile: qr_profile,
      whatsapp: qr_whatsapp
    }
  }
  res.body = { profile: public_data }.to_json
end
...
mount_api server, '/api/admin/company_orders/approve' do |req, res|
  ...
  elsif action == 'mark_approved'
    order['status'] = 'approved'
  elsif action == 'mark_shipped'
...
# Admin: Get all companies (with stats)
mount_api server, '/api/admin/companies' do |req, res|
  set_cors(req, res)
  if req.request_method == 'OPTIONS'
    res.status = 204; next
  end
  ...
  if req.request_method == 'GET'
    ...
    # Enrich with stats
    enriched = companies.map do |c|
      comp_users = users.select { |u| u['company_id'] == c['id'] }
      total_cards = comp_users.reduce(0) { |sum, u| 
        data = JSON.parse(u['data']) rescue {}
        sum + (data['cards'] || []).length
      }
      # Also count from bulk orders if needed, or just use user cards
      # The prompt asks for "totalCardsOrdered", which might mean bulk orders quantity
      bulk_qty = orders.select { |o| o['companyId'] == c['id'] }.reduce(0) { |sum, o| sum + (o['quantity'] || 0) }
      
      c.merge({
        'totalUsers' => comp_users.length,
        'totalCardsOrdered' => bulk_qty + total_cards # Combining both for now, or just bulk? User said "totalCardsOrdered". I'll provide both or sum.
      })
    end
    
    res.body = { companies: enriched }.to_json
    
  elsif req.request_method == 'POST'
    body = parse_body(req)
    # companyId, companyName, contactEmail, contactPhone, billingAddress, createdAt, status, notes
    
    if [body['companyName'], body['contactEmail']].any? { |v| v.to_s.strip.empty? }
      res.status = 400; res.body = { error: 'missing_fields' }.to_json; next
    end
    
    companies = read_companies
    if companies.any? { |c| c['email'] == body['contactEmail'] || c['name'] == body['companyName'] }
      res.status = 409; res.body = { error: 'exists' }.to_json; next
    end
    ...
    new_company = {
      ...
      'address' => body['billingAddress'], # Using address as billing address for simplicity as per prompt mapping
      'billingAddress' => body['billingAddress'],
      'notes' => body['notes'],
      'status' => body['status'] || 'active',
      'createdAt' => Time.now.iso8601,
      # Legacy fields support
      'contactPerson' => body['contactPerson'] || 'Admin',
    ...
    res.body = { ok: true, company: new_company }.to_json
  end
end

# Admin: Update/Delete Company
mount_api server, '/api/admin/companies/manage' do |req, res|
  set_cors(req, res)
  if req.request_method == 'OPTIONS'
    res.status = 204; next
  end
  ...
  body = parse_body(req)
  id = body['id']
  action = body['action'] # 'update', 'delete', 'suspend', 'activate'
  ...
  if action == 'delete'
    companies.delete_if { |c| c['id'] == id }
    # Optional: logic to detach users or delete them
    write_companies(companies)
    res.body = { ok: true }.to_json
    
  elsif action == 'update'
    company['name'] = body['companyName'] if body['companyName']
    company['email'] = body['contactEmail'] if body['contactEmail']
    company['phone'] = body['contactPhone'] if body['contactPhone']
    company['billingAddress'] = body['billingAddress'] if body['billingAddress']
    ...
    res.status = 400; res.body = { error: 'invalid_action' }.to_json
  end
end

# Admin: Get Company Users
mount_api server, '/api/admin/companies/users' do |req, res|
  set_cors(req, res)
  if req.request_method == 'OPTIONS'
    res.status = 204; next
  end
  ...
  company_id = req.query['companyId'] || parse_body(req)['companyId']
  
  if !company_id
     # Try to get from query string manually if WEBrick doesn't parse it well in req.query
     uri = URI.parse(req.request_uri.to_s)
     params = URI.decode_www_form(uri.query || '').to_h
     company_id = params['companyId']
  end

  unless company_id
    res.status = 400; res.body = { error: 'missing_company_id' }.to_json; next
  end
  
  comp_users = users.select { |u| u['company_id'] == company_id }.map { |u| sanitize_user(u) }
  res.body = { users: comp_users }.to_json
end
